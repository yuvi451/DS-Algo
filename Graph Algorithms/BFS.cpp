...
// visit root in the begining
// as you explore the children of a node it might be such that a given child has multiple parents in the current level, so if you don't visit the child the first time you see it, then that child might get pushed into the queue multiple times. 
// Although it will get processed only once but the queue might get filled with redundant values intially !!
